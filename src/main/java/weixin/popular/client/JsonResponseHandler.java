...
public class JsonResponseHandler{
	...
	public static <T> ResponseHandler<T> createResponseHandler(final String charsetName, final Class<T> clazz){
			...
			ResponseHandler<T> responseHandler = new ResponseHandler<T>() {
				...
				public T handleResponse(HttpResponse response)
	                ...
	                if (status >= 200 && status < 300) {
	                    ...
	                    return JsonUtil.parseObject(new String(str.getBytes(charsetName),"utf-8"), clazz);
	                } else {
